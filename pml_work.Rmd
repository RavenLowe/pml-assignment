---
title: "Predictive Modeling on Human Activity Recognition of Weight Lifting Exercise"
output: 
  html_document:
    keep_md: true
    toc: true
...
```{r setoptions & loadLibraries, echo=FALSE}
# set the global options per instruction to present all the code chunks
library(knitr)                                       # R markdown doc
opts_chunk$set(message=FALSE, warning=FALSE,
               echo=TRUE, cache=TRUE,
               fig.path="figure/")                   # set the global options
suppressPackageStartupMessages(library(ggplot2))     # make the plots
suppressPackageStartupMessages(library(gridExtra))   # for multi-panel plots
```
...
```{r, fig.width=5, fig.height=3, fig.align='center', results='hide'}
...
Cross-validating the model results in an estimated error of `r paste(round(100 - cm_rf_probe$overall["Accuracy"]*100, 2), "%")`.  As for the 95% confidence interval: [`r round(cm_rf_probe$overall["AccuracyLower"], 4)`, `r round(cm_rf_probe$overall["AccuracyUpper"], 4)`]. The estimated accuracy is well above the "no information rate" statistic of `r round(cm_rf_probe$overall["AccuracyNull"], 4)`. The validation results also in a high kappa statistic of `r round(cm_rf_probe$overall["Kappa"], 4)`, which suggest a very good classifier.
